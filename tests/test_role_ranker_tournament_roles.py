...
from vct_ranker.roles import enrich_player_roles, infer_player_role, infer_player_role_details
...
def test_stable_five_team_moves_duplicated_sentinel_hybrid_to_flex():
    df = pd.DataFrame(
        [
...
def test_stable_five_team_can_skip_flex_only_with_two_locked_duelists():
    df = pd.DataFrame(
        [
            {"player": "duelist_a", "team": "AAA", "event_id": 1, "agents": "jett,raze"},
...
def test_sen_like_double_sentinel_controller_hybrid_becomes_flex():
    df = pd.DataFrame(
        [
